				...
				xhr.onreadystatechange = function(){
					if(xhr.readyState ==4)
					{
						...
						if(s=='1')
						{
							...
							if(t)
							{
								window.location.href="/index.php/Stu/add_stu1/c_id/";
...
<body style="background:#F0F2F5;">
	<div class="container">
		...
		<form action="/index.php/Stu/addStu_info" method="post" id="form1" class="form-inline" enctype="multipart/form-data" >
			...
			<div class="left_info">
				<table>
					<tr><td><label >学号：</label></td><td><input type="text" name="stu_num"  class="form-control" placeholder="请输入学生学号" ></td>
					<td id="num" style="color:red;font-size:0.65em;">&nbsp;&nbsp;*&nbsp;学号必须唯一</td></tr>
					...
					<tr><td><label >班级：</label></td><td>
					 <select name="class" >
					 <option value="<?php echo ($info[0]["c_id"]); ?>"><?php echo ($info[0]["c_name"]); ?></option></foreach></select><span style="color:red">&nbsp;&nbsp;*</span></td><td>&nbsp;</td>
					<td><label >性别：</label></td><td><select name="sex" onblur="change_img(this.value)"><option value="男">男</option><option value="女">女</option></select></td></tr>
					<tr><td><label >出生年月：</label></td><td><input type="text" id="test1" name="brith" class="form-control" placeholder="请输入出生年月"  >
					<script>
						laydate({
						  elem: '#test1',
						  max: laydate.now(),
